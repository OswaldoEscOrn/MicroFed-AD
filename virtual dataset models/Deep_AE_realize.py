...
def train_autoencoder(X_train, X_val, encoding_dim=32, epochs=80):
    ...
    # Train model
    print("\n🚀 Starting training of Deep AutoEncoder...")
...
def main():
    ...
    virtual_samples_path = os.path.join(save_dir, "virtual_samples_35040.npy")
    ...
    metadata = {
        ...
        "description": "35,040 virtual samples generated by DeepAE "
    }

    metadata_path = os.path.join(save_dir, "virtual_samples_metadata.json")
    ...
    print(f"  Total Samples: {n_samples:,} (matches 35,040 in paper)")
    ...
    # Use all 35,040 samples for evaluation
    ...
    # 8. Visualize results (using all 35,040 samples)
    print("\n📊 Generating visualization for all Deep_AE samples:")
    visualize_results(
        history, X_all, all_reconstructed, all_errors, threshold,
        feature_mapping, model_name="Deep AutoEncoder (35040 samples)"
    )
...
def save_models(autoencoder, encoder, decoder, save_path):
    ...
    # Save as HDF5 format
    autoencoder.save(f"{save_path}autoencoder_model.h5")
    encoder.save(f"{save_path}encoder_model.h5")
    decoder.save(f"{save_path}decoder_model.h5")

    print(f"\n💾 Deep_AE training results saved to: {save_path}")
    print(f"  - autoencoder_model.h5")
    print(f"  - encoder_model.h5")
    print(f"  - decoder_model.h5")
