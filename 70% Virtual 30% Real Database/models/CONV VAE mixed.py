...
MODEL_PATH = r"D:\Oswaldo's surf project\70% Virtual 30% Real Database_Hourly\models\conv_vae_multi_modal.h5"
...
class VAELossLayer(Layer):
    ...
    def call(self, inputs):
        ...
        # Calculate reconstruction loss - fixed: remove unnecessary scaling
        reconstruction_loss = K.mean(K.abs(x_true - x_pred))  # Take mean directly
...
def create_comprehensive_visualization():
    ...
    # Ensure plot_df has enough rows
    plot_df = df_hourly.iloc[:n_plot].copy() if len(df_hourly) >= n_plot else df_hourly.copy()
    plot_anom = df_anomalies.iloc[:min(n_plot, len(df_anomalies))].copy()
    ...
    if len(FEATURE_NAMES) > 0:
        ...
        # Use intersection to avoid KeyError
        ...
        if len(valid_anom_idx) > 0:
            ax5.scatter(valid_anom_idx, plot_df.loc[valid_anom_idx, feat],
                        color='red', marker='o', s=40, label='Anomaly', alpha=0.7)
